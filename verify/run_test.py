...
DATA_FILE = Path("E:/hermes/mflow/data/preference_log.json")
...
def record_conf(choice_idx, confidence):
    """Record with continuous confidence score (0-10) for SINDy analysis."""
    if not (0 <= choice_idx < len(QUESTIONS)):
        print(f"choice must be 0-{len(QUESTIONS)-1}")
        return
    if not (0 <= confidence <= 10):
        print("confidence must be 0-10")
        return
    log = []
    if DATA_FILE.exists():
        log = json.loads(DATA_FILE.read_text())
    ...
    log.append(entry)
    DATA_FILE.write_text(json.dumps(log, indent=2))
    print(f"Session {len(log)}: {confidence}/10 on {QUESTIONS[choice_idx][0]} vs {QUESTIONS[choice_idx][1]}")
    print(f"  → chosen: {QUESTIONS[choice_idx][0] if entry['choice'] % 2 == 0 else QUESTIONS[choice_idx][1]}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: run_test.py record <0-3> [confidence] | analyze")
        sys.exit(1)
    if sys.argv[1] == "record":
        if len(sys.argv) >= 4 and sys.argv[3].isdigit():
            record_conf(int(sys.argv[2]), float(sys.argv[3]))
        else:
